...
?>
<section id="dailyquest" class="mg-activity " style='padding-top: 5px;'>
        ...
        <div class="container-fluid compress-80">
            ...
            <div class="row mg-daily-activity-wrapper clearfix">
                <div class="col-md-8 col-md-offset-2 text-center">
                    ...
                    <a href="<?php echo site_url("activity/10/start"); ?>" class="mg-btn-danger mg-btn-rounded mg-next text-center text-uppercase next-level"><?php echo @$_translation->array[$_lang_map]->item[26]; ?></a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function callnextquestionpull() {
        ...
        if(orderNumber >= activityCount)  {
            ...
            $.post("<?php echo site_url("webapp/saveactivitylinkage"); ?>",{ score: score, points:points, activity_id: activityId, act_link: activityLinkage,scorearray:scorearray,pointsarray:pointsarray,answersarray:answersarray,canswersarray:canswersarray  },function( data ) {
                $(".main-activity-div").addClass('hide');
                ...
                <?php
                    if($_from == "rhymes") {
                ?>
                        var urrl = "<?php echo site_url("/stories/rhymes"); ?>";
                        $(".next-level").attr('href',urrl);
                        $(".next-level").html('<?php echo @$_translation->array[$_lang_map]->item[28]; ?>');
                <?php
                    } else {
                ?>
            ...
            $.post("<?php echo site_url("webapp/getactivitylinkage"); ?>",{num: orderNumber,activityCount:activityCount, activity_id: activityId, act_link: activityLinkage[orderNumber]  },function( data ) {
